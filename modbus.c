...
void function_rtu_handler(void)
{
  ...
  case 0x03: //Read_Holding_Registers
    {
    short first_addr = (request_command[2]<<0x08) | request_command[3];
    short quantity  = (request_command[4]<<0x08) | request_command[5];
    rp.byte_count = 0;
    rp.size = 3;
    for(short i=first_addr; i < quantity  + first_addr; i++)
    {
      signed short temp = (signed short)read_eeprom(i);
    ...
    short crc_cal = CRC16(rp.data, rp.size);
    rp.size+=2;
  ...
  case 0x06: //Write_Single_Register
    {
    short register_addr = request_command[2]<<0x08;
    register_addr |= request_command[3];
    short wr_data =  request_command[4]<<0x08;
  ...
  case 0x10: //Write_Multiple_Registers
    {
    short first_addr = request_command[2]<<0x08;
    first_addr |= request_command[3];
    short quantity  = request_command[4]<<0x08;
    quantity  |= request_command[5];
    //short _byte_count = request_command[6];
    short i=7;
    for(short _addr=first_addr; _addr<first_addr + quantity; _addr++ )
    {
      short wr_data =  request_command[i]<<0x08;
...
{
  *_short  = hex(_key1) <<0xC;
  *_short |= hex(_key2) <<0x8;
  *_short |= hex(_key3) <<0x4;
  *_short |= hex(_key4);
}
//Output Coils==================================================================
...
{
  short first_addr;
  asc4_to_short(&first_addr 
  ,request_command[4]
  ,request_command[5]
  ,request_command[6]
  ,request_command[7]);
  short num_addr;
  asc4_to_short(&num_addr 
  ,request_command[8]
  ,request_command[9]
  ,request_command[10]
  ,request_command[11]);
  unsigned char j=6;
  rp.byte_count = 0;
  for (short i=first_addr; i < first_addr + num_addr 
                        && i <= max_eep_addr; i++)
  {
    short_to_4asc(read_eeprom(i),
